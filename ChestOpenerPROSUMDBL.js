...
(()=>{
  ...
  summary.textContent = 'Chest Opener v1.2c改 プロフェッショナル ATK&DEF合算値Ver.';
  ...
  (()=>{
    ...
    for(const v of ranks){
      ...
      const inputLeft = input.cloneNode();
      inputLeft.dataset.rank = v;
      inputLeft.placeholder = '武器';

      const inputRight = input.cloneNode();
      inputRight.dataset.rank = v;
      inputRight.placeholder = '防具';

      label.append(span_, inputLeft, inputRight);
      div.append(label);
    ...
    inputLeft.addEventListener('input', handleInput);
    inputRight.addEventListener('input', handleInput);
  }
