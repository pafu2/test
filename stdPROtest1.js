...
function isBefore4AM() {
  return new Date().getHours() < 4;
}
async function attackRegionBefore4() {
      while(dialog.open) {
        let success = false;
          ...
          try {
            ...
            if (success) {
              ...
              if (currentProgress < 50) {
                ...
                nextProgress = Math.floor(Math.random() * 8) + 35; // 15~22 -2~+1
              }
              }
              next = `→ ${nextProgress}±2%`;
...
async function attackRegionAfter4() {
      while(dialog.open) {
        let success = false;
        isAutoJoinRunning = true;
