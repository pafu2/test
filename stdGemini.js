...
(()=>{
  ...
  async function autoJoin() {
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      let cellType;
      if (regions.unoccupied.length > 0) {
        cellType = 'unoccupied';
      } else if (regions.enemy.length > 0) {
        cellType = 'enemy';
      } else if (regions.enemyCapital.length > 0) {
        cellType = 'enemyCapital';
      } else if (regions.ownTeam.length > 0) {
        cellType = 'ownTeam';
      } else {
        // 全てのリストが空の場合（通常は起こり得ませんが、安全策として）
        return; 
      }
          ...
          try {
            ...
            if (messageType === 'capitalAttack') {
              ...
              if (messageType === cellType) {
                processType = 'continue';
              } else if (messageType === 'unoccupied') {
                cellType = 'unoccupied';
                processType = 'break';
              } else if (messageType === 'enemy') {
                cellType = 'enemy';
                processType = 'break';
              } else if (messageType === 'enemyCapital') {
                cellType = 'enemyCapital';
                processType = 'break';
              }
              i++;
    ...
    async function getRegions () {
      try {
        ...
        // 2. 攻撃可能な座標の判定
        // 首都（自チームのセル）がない場合は全域、ある場合は隣接＋自陣
        let reachableSet = new Set();
        if (teamColorSet.size === 0) {
          ...
          for (const key of teamColorSet) {
            reachableSet.add(key); // 自チーム内（味方撃ち用）
            const [tr, tc] = key.split('-').map(Number);
            for (const [dr, dc] of directions) {
              const nr = tr + dr;
              const nc = tc + dc;
              if (nr >= 0 && nr < rows && nc >= 0 && nc < cols) {
                reachableSet.add(`${nr}-${nc}`);
              }
            }
          }
        }

        const unoccupied = [];    // 1. まだどこのチームにも属してない
        const enemy = [];         // 2. 敵チーム（首都除く）
        const enemyCapital = [];  // 3. 敵チーム（首都）
        const ownTeam = [];       // 4. 自チーム

        for (const key of reachableSet) {
          const [r, c] = key.split('-').map(Number);
          const cellData = cellColors[key];

          if (!cellData) {
            unoccupied.push([r, c]);
          } else if (teamColorSet.has(key)) {
            ownTeam.push([r, c]);
          } else {
            // 敵チームのセル
            if (capitalSet.has(key)) {
              enemyCapital.push([r, c]);
            } else {
              enemy.push([r, c]);
            }
          }
        ...
        // 優先順位順に格納したオブジェクトを返す
        return {
          unoccupied: shuffle(unoccupied),
          enemy: shuffle(enemy),
          enemyCapital: shuffle(enemyCapital),
          ownTeam: shuffle(ownTeam)
        };
      } catch (e) {
        console.error(e);
        return;
