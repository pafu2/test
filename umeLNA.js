...
(()=>{
  ...
  (()=>{
    ...
    (()=>{
      ...
      link.textContent = '1.2.2d改 ume明け方予約版';
      footer.append(link);
    })();
...
if (regions.nonAdjacent.length > 0) {
  ...
  cellType = 'mapEdge';
} else if (regions.emergency?.length > 0) {
  cellType = 'emergency';
} else {
  cellType = null; // 本当に何もない場合
}
...
//            logMessage(null, '[打止] 終了です。', next);
    ...
    async function getRegions () {
      try {
        ...
        //誰もいない夜中狙い
          const uncolored = cells.filter(([r, c]) => {
            const key = `${r}-${c}`;
            return !cellColors.hasOwnProperty(key);
          });
        ...
        //誰もいない夜中狙い
const enemyCapitalCells = capitalMap.filter(([r, c]) => {
  const key = `${r}-${c}`;
  return cellColors[key] !== teamColor;
});
...
const hasAnyTarget = Object.values(regions).some(arr => arr.length > 0);

// ★ 最終手段
if (!hasAnyTarget && enemyCapitalCells.length > 0) {
  regions.emergency = shuffle(enemyCapitalCells);
}
        return regions;
      } catch (e) {
        console.error(e);
