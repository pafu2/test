...
// @version      1.2.2d.パクリ9.4改 連射版 - 5発撃ちfullspec
 ...
 if (MODEQ === 'm=l') {
  ...
  (()=>{
    ...
    (()=>{
      ...
      link.textContent = 'arena assist tool - v1.2.2d.パクリ9.4改 連射版 - 5発撃ちfullspec';
  ...
  async function autoJoin() {
    ...
    async function getRegions () {
      try {
        ...
        let teamAdjacentCells = cells.filter(([r, c]) => {
          const key = `${r}-${c}`;
          return teamColorSet.has(key) || teamAdjacentSet.has(key);
        });

        // ←ここに追加
        teamAdjacentCells = shuffle(teamAdjacentCells).sort((a, b) => {
          const aKey = `${a[0]}-${a[1]}`;
          const bKey = `${b[0]}-${b[1]}`;
          const isATeam = teamColorSet.has(aKey);
          const isBTeam = teamColorSet.has(bKey);
          return (isATeam ? 1 : 0) - (isBTeam ? 1 : 0);
        });
        ...
        const regions = {
          ...
          teamAdjacent: teamAdjacentCells, // 上書き済み
