...
const BASE_URL = location.hostname.endsWith('5ch.io')
                 ? 'https://donguri.5ch.io'
                 : 'https://donguri.5ch.net';

(()=>{
  if(location.href === '${BASE_URL}/bag') {
    ...
    tableIds.forEach((elm, index)=>{
      const equipLinks = document.querySelectorAll(`#${elm} a[href^="${BASE_URL}/equip/"]`);
  ...
  (()=>{
    ...
    async function showEquipList(){
      if(!weaponTable || !armorTable || !necklaceTable) {
        try {
          const res = await fetch('${BASE_URL}/bag');
          ...
          [weaponTable,armorTable,necklaceTable].forEach((table,index) => {
            ...
            rows.forEach(row => {
              const id = row.cells[1].querySelector('a')?.href.replace('${BASE_URL}/equip/','');
  ...
  async function setPresetItems (presetName) {
    ...
    const fetchPromises = equipPresets[presetName].id
      ...
      .map(id => fetch('${BASE_URL}/equip/' + id));
  ...
  async function fetchSingleArenaInfo(elm) {
    try {
      ...
      const url = `${BASE_URL}/teambattle?r=${row}&c=${col}&`+MODE;
