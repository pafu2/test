...
async function autoJoin() {
  ...
  function getMessageType(text) {
    return Object.keys(messageTypes).find(key =>
      messageTypes[key].some(v => text.includes(v))
    );
  }
  ...
  async function attackRegion() {
    ...
    if (isAutoJoinRunning || Math.abs(nextProgress - currentProgress) >= 3) return;
    ...
    while (dialog.open) {
      ...
      for (let i = 0; i < regions[cellType].length;) {
        ...
        try {
          ...
          if (success) {
            if (currentProgress < 15) nextProgress = 20;
            else if (currentProgress < 25) nextProgress = 30;
            else if (currentProgress < 35) nextProgress = 40;
            else if (currentProgress < 50) nextProgress = 60;
            else if (currentProgress < 65) nextProgress = 70;
            else if (currentProgress < 75) nextProgress = 80;
            else if (currentProgress < 85) nextProgress = 90;
            else nextProgress = 10;
            next = `→ ${nextProgress}±2%`;
