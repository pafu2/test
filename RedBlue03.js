...
(()=>{
  ...
  if (grid && grid.parentNode) {
    ...
    // gridレイアウトの土台として再定義する
    grid.style.display = 'grid';
    grid.style.justifyContent = 'center';
  }

  const table = document.querySelector('table');
  if (table && table.parentNode) { // tableが存在しない場合のエラー防止
...
// const を消して、既存の変数に代入する形にする（あるいは単に grid を使う）
...
async function refreshArenaInfo() {
    ...
    try {
      ...
      // 全てのスクリプトを連結して、変数 cellColors, capitalList, GRID_SIZE, terrainsPayload を探す
...
async function fetchAreaInfo(refreshAll){
    ...
    // 大元の grid が定義されていれば、ここでの ReferenceError は消えます
    if (currentViewMode === 'detail') {
      // 念のため grid が存在するかチェックする
      if (grid) {
...
cell.style.zIndex = '100';
      ...
      cell.style.position = 'relative'; // z-indexを有効にするために必要です
...
function toggleCellViewMode () {
  // すでに他で定義されているはずなので、const を付けずに中身を確定させる
  const grid = document.querySelector('.grid') || document.querySelector('#gridWrap');
  if (!grid) return;
  const cells = grid.querySelectorAll('.cell');
  // ...（以下、お渡し済みの toggleCellViewMode の中身）

    // マスの総数から一辺のマス数を計算（例: 25マスなら5x5）
    const count = Math.sqrt(cells.length);

    if(currentViewMode === 'detail') {
      currentViewMode = 'compact';

      // replaceに頼らず、直接数値を指定して再定義する
...
async function getRegions () {
      try {
        ...
        const waterSet = new Set(); // ★水マスを記録するセットを追加
        ...
        if (location.href.includes('m=rb')) {
          ...
          // hc仕様では水マスの概念がない（または別の方法）ため、ここでは空のままになります
        }
        ...
        // 各カテゴリーのフィルタリング時に !waterSet.has(key) を追加
