  ...
  async function autoJoin() {
    ...
    async function getRegions() {
      try {
        ...
        // 除外対象のセルを取り除くフィルタリング関数
        function excludeTeamColorCells(cellSet, colorSetToExclude) {
          return cellSet.filter(([r, c]) => {
            const key = `${r}-${c}`;
            return !colorSetToExclude.has(key); // 除外対象カラーのセルを除外
          });
        }

        // nonAdjacentCells, capitalAdjacentCells, mapEdgeCellsから特定のカラーのセルを除外
        const filteredNonAdjacentCells = excludeTeamColorCells(nonAdjacentCells, teamColorSet);
        const filteredCapitalAdjacentCells = excludeTeamColorCells(capitalAdjacentCells, teamColorSet);
        const filteredMapEdgeCells = excludeTeamColorCells(mapEdgeCells, teamColorSet);
        ...
        const regions = {
          nonAdjacent: shuffle(filteredNonAdjacentCells), // 最優先
          capitalAdjacent: shuffle(filteredCapitalAdjacentCells), // 敵の首都を除いたセル
          mapEdge: shuffle(filteredMapEdgeCells), // 敵の首都を含むマス
          teamAdjacent: shuffle(teamAdjacentCells) // 自チームのマス
        };
