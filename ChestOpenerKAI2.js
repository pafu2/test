...
(()=>{
  ...
  // battle chest
  (()=>{
const forceLockHighRank = document.createElement('input');
forceLockHighRank.type = 'checkbox';
forceLockHighRank.checked = true;

const label = document.createElement('label');
label.append(forceLockHighRank, 'Pt / Au を常にロック（条件無視）');
battleChestField.prepend(label);  // battleChestField はすでに定義済み

function toggleBattleSelects(disabled) {
  document.querySelectorAll('.min-buffs,.max-debuffs')
    .forEach(elm => elm.disabled = disabled);
}

// 初期状態でチェックに合わせてプルダウンを無効化
toggleBattleSelects(forceLockHighRank.checked);

// チェックボックス変更時に連動
forceLockHighRank.addEventListener('change', e => {
  toggleBattleSelects(e.target.checked);
});
        ...
        try {
          ...
          if(h1.textContent.includes('アイテムバッグ')){
            ...
            if(!shouldNotRecycle.checked){
              ...
              // 分解
if (window.forceLockHighRank?.checked && (itemRank === 'Pt' || itemRank === 'Au')) {
  // チェックONかつ Pt/Au の場合 → 無条件ロック
  try {
    const lockLink = lastItem.querySelector('a[href^="https://donguri.5ch.net/lock/"]');
    if (lockLink) await fetch(lockLink.href, { method: 'GET' });
  } catch (error) {
    forceStop(error);
    break;
  }
