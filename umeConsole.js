...
//      timestamp.innerText = `${ymd}\n${time}`;
    ...
    if (location.href.includes('/teambattle?m=rb')) {
      ...
      let loop = 0;
console.log('regions:', regions);  // regions の状態をログに出力
      let cellType;
...
console.log('Selected cellType:', cellType);  // 最終的に選ばれた cellType をログに出力
      while(dialog.open) {
        let success = false;
        isAutoJoinRunning = true;
console.log('Before filter, regions[cellType]:', regions[cellType]);
