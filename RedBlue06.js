...
(()=>{
  ...
  async function fetchAreaInfo(refreshAll){
    ...
    if (currentViewMode === 'detail') {
      if (grid) {
        grid.style.gridTemplateRows = grid.style.gridTemplateRows.replace('35px','55px');
        grid.style.gridTemplateColumns = grid.style.gridTemplateColumns.replace('35px','85px');
      }
    }
    if (grid && grid.parentNode) {
      grid.parentNode.style.height = null;
  ...
  async function fetchSingleArenaInfo(elm) {
    try {
      ...
      //全角5文字でカット
      const displayLeader = getCutName(leader, 10);
      ...
      if (currentViewMode === 'detail') {
        ...
        p[1].textContent = displayLeader;
        p[0].style.margin = '0';
        p[1].style.margin = '0';
        p[0].style.fontSize = '11px';
        p[1].style.fontSize = '12px';
        p[1].style.whiteSpace = 'nowrap';
  ...
  function getCutName(str, limit = 10) {
    if (!str) return "";
    let count = 0;
    let res = "";
    for (const char of str) {
      // 全角なら+2、半角なら+1
      count += char.match(/[^\x01-\x7E\xA1-\xDF]/) ? 2 : 1;
      if (count <= limit) {
        res += char;
      } else {
        break;
  ...
  function toggleCellViewMode () {
    ...
    if(currentViewMode === 'detail') {
      ...
      grid.style.gridTemplateRows = `repeat(${count}, 55px)`;
      grid.style.gridTemplateColumns = `repeat(${count}, 85px)`;
