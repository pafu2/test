...
const group1 = shuffle(nonAdjacentBase.filter(([r, c]) => {
  ...
  console.log(`Checking group1 for cell ${key} with rank ${rank}`);  // デバッグログ
  if (rank?.includes('警')) {
    console.log(`Excluding ${key} from group1 because it's a police rank`);  // 警察ランク除外ログ
    return false;
  }
...
const group2 = shuffle(nonAdjacentBase.filter(([r, c]) => {
  const key = `${r}-${c}`;
  const rank = rankMap.get(key);
  console.log(`Checking group2 for cell ${key} with rank ${rank}`);  // デバッグログ
  if (rank?.includes('警')) {
    console.log(`Excluding ${key} from group2 because it's a police rank`);  // 警察ランク除外ログ
    return false;
  }
...
const group3 = shuffle(cells.filter(([r, c]) => {
  const key = `${r}-${c}`;
  const rank = rankMap.get(key);
  console.log(`Checking group3 for cell ${key} with rank ${rank}`);  // デバッグログ
  if (rank?.includes('警')) {
    console.log(`Excluding ${key} from group3 because it's a police rank`);  // 警察ランク除外ログ
    return false;
  }
...
const group4 = shuffle(cells.filter(([r, c]) => {
  const key = `${r}-${c}`;
  const rank = rankMap.get(key);
  console.log(`Checking group4 for cell ${key} with rank ${rank}`);  // デバッグログ
  if (rank?.includes('警')) {
    console.log(`Excluding ${key} from group4 because it's a police rank`);  // 警察ランク除外ログ
    return false;
  }
  if (!capitalSet.has(key)) return false;
