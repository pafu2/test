...
// @version      1.2.2d.パクリ9.4改 連射版 - hoge3
 ...
 if (MODEQ === 'm=l') {
  ...
  (()=>{
    ...
    (()=>{
      ...
      link.textContent = 'arena assist tool - v1.2.2d.パクリ9.4改 連射版 - hoge3';
...
//              }
                ...
                if (!capitalExpanded) {
                    ...
                    for (let [dr, dc] of directions) {
                        if (filled >= 44) break;//首都上下左右の5マス以外に埋めるマス数44（例は7×7なので計49マス、首都と上下左右の5マス引いて44マスなので44）
                        const nr = r + dr;
                        const nc = c + dc;
                        if (nr >= 0 && nr < rows && nc >= 0 && nc < cols) {
                            try {
                                await challenge([nr, nc]);
                                filled++;
                                message = `(${filled}発目) 首都周囲のマスを埋めました！`;
                                logMessage('首都拡張', message, '次回予約へ');
                                await sleep(1.5);// 1.5秒待機（必要に応じて調整）
                            } catch(e){}
                        }
                    }
                    capitalExpanded = true;
                }
                if (loop < 9){
                  loop += 1;
