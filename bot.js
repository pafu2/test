...
async function attackRegion() {
  ...
  // ★ここを追加：開始検知フラグが立っていたら、%の差を無視して進む
  const forceStart = isGameStart(latestText); // latestTextはfetchした中身

  if (!forceStart) { // 開始の瞬間でない時だけ、いつもの % 判定をする
...
async function drawProgressBar(){
    try {
      ...
      // ★追加：リセット検知（計算より先にチェック）
      if (isGameStart(text)) {
          console.log("後半戦または新期の開始を検知しました");
          startAutoJoin(); 
          // 0%や50%の時は、後の計算処理を待たずにここで抜ける
          return; 
      }
...
// 初回実行
  ...
  let currentWaitTime = 18000;

  // 進捗に合わせて監視速度を切り替える関数
  function updateProgressBarTimer() {
    // 49%, 50%, 99%, 0% の時は 0.7秒に加速（bot対策）
    let nextWaitTime = 18000;
    if (currentProgress === 49 || currentProgress === 50 || currentProgress === 99 || currentProgress === 0) {
      nextWaitTime = 700; 
    }

    // 待ち時間が変わる、またはタイマーが止まっている場合に再設定
    if (nextWaitTime !== currentWaitTime || !progressBarIntervalId) {
...
function isGameStart(htmlText) {
    // cellColorsが空、またはcapitalMapが空、またはグリッドサイズが変化した瞬間を捉える
    return htmlText.includes('const cellColors = {  };') || 
           htmlText.includes('const capitalMap = [];');
  }
