...
// @version      1.2.2d.パクリ9.2改 2撃仕様(早撃ち)
 ...
 if (MODEQ === 'm=l') {
  ...
  (()=>{
    ...
    (()=>{
      ...
      link.textContent = 'arena assist tool - v1.2.2d.パクリ9.2改 2撃仕様(早撃ち)';
...
nextProgress = Math.floor(Math.random() * 4 ) + 35;
} else if (currentProgress < 50) {
nextProgress = Math.floor(Math.random() * 4 ) + 56;
} else if (currentProgress < 75) {
nextProgress = Math.floor(Math.random() * 4 ) + 85;
} else {
nextProgress = Math.floor(Math.random() * 4 ) + 7;
}
              next = `→ ${nextProgress}±5%`;
