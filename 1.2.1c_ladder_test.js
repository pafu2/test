...
(()=>{
    ...
    (()=>{
      ...
      (()=>{
        ...
        intervalInput.addEventListener('input', ()=>{
          ...
          settings.autoJoinInterval = interval > 180 ? interval : 180;
          localStorage.setItem('aat_settings', JSON.stringify(settings))
        })
        ...
        p.textContent = 'この画面を開いたままにしておくこと。最短180秒';
        p.style.margin = '0';
  ...
  async function arenaChallenge (row, col){
    ...
    try {
      const response = await fetch('/teamchallenge?m=l', options);
      if(!response.ok){
