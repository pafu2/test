...
//grid.style.gridTemplateRows = `repeat(${rows}, 1fr)`;// 高さを比率に
//grid.style.gridTemplateColumns = `repeat(${cols}, 1fr)`;// 幅を比率に
//grid.style.width = '100%';//親要素
//grid.style.maxWidth = '600px';//上限
      grid.style.gap = '2px';
      grid.style.justifyContent = 'center';
  ...
  async function getRegions () {
      try {
        ...
        const group3 = shuffle(cells.filter(([r, c]) => {
          ...
          return color !== teamColor && !waterSet.has(key);
        }));
